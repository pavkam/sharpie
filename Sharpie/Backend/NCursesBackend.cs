...
internal class NCursesBackend: BaseCursesBackend
{
    ...
    /// <inheritdoc cref="BaseCursesBackend.DecodeRawMouseButtonState"/>
    protected internal override (MouseButton button, MouseButtonState state, ModifierKey modifierKey) DecodeRawMouseButtonState(uint flags)
    {
        _cursesMouseEventParser ??= CursesMouseEventParser.Get(mouse_version());
        var parsed = _cursesMouseEventParser.Parse(flags);
